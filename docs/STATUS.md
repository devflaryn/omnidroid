...
| M6 Vulkan device created through the forwarding layer | **Not reached; the host half is built and the guest half is not.** §8 rows 16-20 are reached and **row 21's flags load**: `nativeInitClientSettings` returns 0, `Flag::areFlagsLoaded` goes to 1, and the engine writes its flag cache, brings up Mimalloc and reports its build. `nativePostClientSettingsLoadedInitialization3` then blocks in a one-shot `pthread_cond_wait` reached through `getFlags("ClientAppSettings")` -- opt-in behind `OMNI_M6_ROWS_21_22=1`. The previous account of that block (two futex-parked workers that never signal) was **wrong and is withdrawn**: the census had been switched off, so "FROZEN" was not a reading at all, and the un-gated `Boundary::crossings().exits` showed the guest executing 1.3 million imports a second. What was actually happening is that three guest worker threads had been **killed by this layer** -- one by a string walk bounded by a commit granule (`omni_mem::scan_reach` now), two by unbound `pthread_getattr_np`/`pthread_mutex_trylock` -- and one of them held the future the call waits on. The gate now drives row 21's first downcall **before** rows 17-20, because the engine says so in its own words (`nativeActivity_onSurfaceChanged: ... Flags-Not-Received. Return.`), and with the string fix the engine reaches its settings fetch and names the URL it wants. **21 of 21** scripted downcalls now return. On the host side `omni-platform::window` and `omni-gfx` exist and were verified: a resizable Win32 window and a Vulkan swapchain whose frames were read back off the screen. **The guest half is still nothing**: 0 of the 17 `egl*` symbols are bound, there are 0 `vk*` imports (Vulkan arrives via `dlopen("libvulkan.so")`, which `bionic/dl.rs` refuses), nothing in `omni-android` calls `omni-gfx`, and `ANativeWindow` geometry is a constant the gate supplies rather than a window's |
