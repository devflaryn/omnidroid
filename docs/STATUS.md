...
| M6 Vulkan device created through the forwarding layer | **Not reached; begun.** §8 **rows 16-20 are reached**: all seven lifecycle/surface natives return and the engine logs `APP_CMD_INIT_WINDOW: hasWindow = true` through `APP_CMD_CONTENT_RECT_CHANGED`, then `nativeActivity_onSurfaceChanged: ... Flags-Not-Received. Return.` — §8.1's sixth failure mode, exactly where it says it will be. `ALooper_pollOnce(-1)` now waits rather than refusing, on a measured wake source (a watched pipe with a live write end). **Rows 21-22 are driven** and `nativeInitClientSettings` reaches the engine's settings parser, then spins at guest `0x021eba20` on a lock held since §8 step 7 by one of the two stranded workers above. **What still does not exist: any graphics at all.** `crates/omni-gfx` is a 16-line stub nothing depends on; **0 of the 17 `egl*` symbols are bound**; there are **0 `vk*` imports** (Vulkan arrives via `dlopen("libvulkan.so")`, which `bionic/dl.rs` refuses); `omni-texture` decodes the APK's ETC1 and has no consumer |
