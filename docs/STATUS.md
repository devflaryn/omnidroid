...
| The engine's own threads issue **raw syscalls** | `syscall 98` (arm64 `futex`), from two engine worker threads, MEASURED in M5's gate (n = 1). They bypass every `pthread_*` symbol this layer binds. The refusal is right, and it is the **largest single obstacle between M5 and M6**: it needs a decision, not code |
...
| M6 Vulkan device created through the forwarding layer | **Not reached; begun.** What exists: `ANativeWindow` (the **five** symbols `libroblox.so` imports, not §4.4's nine — arena handles, and a geometry that **refuses** until an embedding calls `Ndk::set_window_geometry`, because there is no surface and `1920x1080` would be a device profile nobody chose); the raw `futex` syscall (`SYS_FUTEX` 98), which removed both `syscall 98` guest-thread deaths; and a superclass chain in the JNI registry, which fixed a null `jmethodID` reaching `CallObjectMethodV` with no `cbz` between them (§8.1 failure mode 3, guest pc `0x02bd8cac`). The game thread now reaches `NativeEngine::GameLoop()` → `MainGameActivity.getNativeHelper()` → `bootstrapTheApp()` and stops in `ALooper_pollOnce(-1)`. **What does not exist: any graphics at all.** `crates/omni-gfx` is a 16-line stub nothing depends on; **0 of the 17 `egl*` symbols are bound**; there are **0 `vk*` imports** (Vulkan arrives via `dlopen("libvulkan.so")`, which `bionic/dl.rs` currently refuses); `omni-texture` decodes the APK's ETC1 and has no consumer. §8 rows 16-20 have no driver |
