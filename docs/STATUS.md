...
| `omni-platform` Linux / macOS | **Not implemented, and does not pretend to be.** Every primitive that needs a *target-specific* call returns a typed "unsupported on this platform" error naming its intended POSIX call — `getrandom(2)`, `arc4random_buf(3)`, `sched_getcpu(3)`, `pread(2)`, `statvfs(3)` — so a non-Windows build fails immediately rather than misbehaving. The primitives that are **one portable `std` call** — the clocks, the log sink, `pid`, `cpu_count`, and fifteen of the seventeen file operations — are implemented once, deliberately without a fabricated refusal (D22, D23). The test is not "does it call the OS" but "is there one `std` call that serves all five targets". **Nothing here has been run on any non-Windows target** |
...
| `omni-android` — the bionic adapter | **Phases 1, 2, 3a, 3b and 3c done, pending review.** M3 task 3. **174 of the 188 statically reachable imports** covered: 156 thunk functions bound and all 18 `STT_OBJECT` data objects placed and filled (D20-D24). **14 left** — eight sockets and polling, and six nothing else claims. `AT_HWCAP` is deliberately still undecided and `getauxval` refuses until a host says which arm |
