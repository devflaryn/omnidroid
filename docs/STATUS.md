...
| M5 `initializeNativeCode` runs, surface requested | **In progress.** Tasks 1 and 2 of `docs/plans/gameactivity-plan.md` are done and mutation-verified: `pipe`, `fcntl` and `write` bound with a real readiness source behind `poll` and `select`, and the seven `ALooper` symbols in a new `omni-android::ndk`. Both of §8.1's silent failure modes now have an instrument pointed at them — `Ndk::prepare_looper` makes the null-looper case a *precondition* a gate asserts before step 13 rather than a `jlong` of 0 read back afterwards, and `Bionic::parked()` names every guest thread blocked in `pthread_cond_wait` with the cond and mutex it holds. Tasks 3-5 (`AAssetManager`, `AConfiguration`, `ANativeWindow`, the gate) are not started |
