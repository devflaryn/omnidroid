...
- **`omnidroid/warmcache.py` imports only `hashlib` and `json` after Task 3** (the rest were removed as dead). Tasks 4-6 append code using `Path`, `shutil` and `time` — each must add the imports it needs at the top of the file. A `NameError` here means you skipped this line.
...
def commit_bake(images_dir, key, tmp, meta):
    ...
    one actually get deleted. Deleting it any earlier means a failed publish
    destroys a previously-working, expensive-to-rebuild entry as well as the
    new one. time_ns() rather than time(): whole seconds collide when the
    same key is committed twice inside one second.
    """
    tmp = Path(tmp)
