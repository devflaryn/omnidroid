//! What loading `libroblox.so` actually *costs* on Linux: the mirror of `tests/loader_commit.rs`.
//!
//! A directory target (`tests/loader_commit_linux/main.rs`) so that `tests/windows_only.rs`, which
//! accounts for every `tests/*.rs` by name, does not see it. Three changes from the Windows file,
//! each for a reason that is Linux's:
//!
//! * **Commit charge is `VM_ACCOUNT`** (`omni_platform::vm::process_commit_charge` on Linux), which
//!   does not include page tables. The ceilings are unchanged, so they are slightly looser here.
//! * **A missing fixture fails.** The Windows file returns early when the APK is absent, which is
//!   VERIFICATION entry 4; the APK is present on the port host, and a skip here would be a green run
//!   that measured nothing.
//! * **Every measurement runs in one `#[test]`**, because each libtest thread that starts on Linux is
//!   2 MiB of `VM_ACCOUNT` -- the size of the tolerance the load/unload cycle asserts.
//!
...
//! `cargo test -p omni-elf --release --test loader_commit_linux -- --nocapture` to read the numbers.
//!
...
fn peak_and_steady_commit_charge_are_measured_and_reported() {
    ...
    let f = fixture().expect(
        "the libroblox.so fixture: a missing fixture is a failure, not a skip (VERIFICATION entry 4)",
    );
...
#[test]
fn loading_libroblox_costs_with_no_other_test_thread_alive() {
    eprintln!("--- peak_and_steady_commit_charge_are_measured_and_reported");
    peak_and_steady_commit_charge_are_measured_and_reported();
    eprintln!("--- loading_and_unloading_repeatedly_returns_all_commit_charge");
    loading_and_unloading_repeatedly_returns_all_commit_charge();
    eprintln!("--- the_relocation_window_bounds_the_transient_charge_and_costs_time");
    the_relocation_window_bounds_the_transient_charge_and_costs_time();
    eprintln!("--- lazy_bss_costs_nothing_until_something_touches_it");
    lazy_bss_costs_nothing_until_something_touches_it();
    eprintln!("--- three_instances_share_the_file_backed_image");
    three_instances_share_the_file_backed_image();
}
