//! **Hostile input for C1** (Global Constraint 11), on Linux: the mirror of
//! `fault_teardown_race.rs`, unchanged but for the gate. Real `SIGSEGV`s arrive on four threads
//! while a handler is torn down, and the registrant's context must never be touched after its
//! registration's `drop` has returned. The quiescence protocol is `fault/windows.rs`'s, copied into
//! `fault/linux.rs`; this is the evidence that the copy holds under a real signal storm and not only
//! under the deterministic `dispatch` tests in that file.
//!
...
fn serve(context: usize, fault: &Fault) -> FaultOutcome {
    ...
    // that another thread committed a moment ago is idempotent (commit is `mprotect` on Linux, which
    // keeps the page's contents) and is the normal outcome of two threads faulting on the same page.
...
fn install(context: &'static Served) -> FaultRegistration {
    ...
    // handled by value); it takes no lock this process holds across a fault, only the vm ledger and `mprotect`;
