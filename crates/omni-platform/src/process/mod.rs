...
mod tests {
    ...
    #[test]
    #[cfg_attr(not(any(target_os = "windows", target_os = "linux")), ignore = "no entropy backend on this target")]
    ...
    #[test]
    #[cfg_attr(not(any(target_os = "windows", target_os = "linux")), ignore = "no process-cpu-time backend on this target")]
    ...
    fn process_cpu_time_is_answered_or_refused_by_name() {
        match cpu_time() {
            Ok(_) => assert!(
                ...
                "only Windows and Linux have a cpu-time backend"
            ),
            Err(error) => {
    ...
    fn the_current_cpu_is_answered_or_refused_by_name() {
        match current_cpu() {
            Ok(_) => assert!(
                cfg!(any(target_os = "windows", target_os = "linux")),
                "only Windows and Linux have a cpu-id backend"
            ),
            Err(error) => {
                assert!(error.is_unsupported(), "{error}");
                let text = error.to_string();
