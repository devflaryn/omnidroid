...
enum class SystemRegisterEncoding : u32 {
    ...
    // Counter-timer Virtual Count register (Omnidroid patch 0001: see patches/)
...
bool TranslatorVisitor::MRS(Imm<1> o0, Imm<3> op1, Imm<4> CRn, Imm<4> CRm, Imm<3> op2, Reg Rt) {
    ...
    // Omnidroid patch 0001: EL0 sees CNTVCT_EL0 = CNTPCT_EL0 - CNTVOFF_EL2, and Linux arm64
    // zeroes CNTVOFF_EL2 when it boots at EL2, so the two counters read the same.
