            ...
            other => Err(refused(
                ...
                &format!(
                    "the guest's `ANativeWindow *` resolved to a {system} window, and this host \
                     ...
                     `VK_KHR_win32_surface`, `VK_KHR_xlib_surface` and `VK_EXT_metal_surface`. This is a \
                     refusal rather than a `VkResult` because the specification has no code for \
                     \"this build of the host cannot make a \
