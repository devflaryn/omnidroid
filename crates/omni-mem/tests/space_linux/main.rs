...
//!
//! **The Linux mirror of `tests/space.rs`**, test for test. It lives in a directory
//! (`tests/space_linux/main.rs`) rather than beside its twin because `tests/windows_only.rs`
//! accounts for every `tests/*.rs` file by name and must not be edited from this port; a directory
//! target is a test binary that list does not see. The only changes are the gate, the path to the
//! shared fixtures, and the one host code that differs (`EACCES` for a read-only descriptor, where
...
fn a_shared_backing_needs_write_access_and_a_non_empty_file() {
    ...
    assert_eq!(
        ...
        Some(13),
        "EACCES, Linux's code for a MAP_SHARED writable mapping of a read-only descriptor: {error}"
    );
