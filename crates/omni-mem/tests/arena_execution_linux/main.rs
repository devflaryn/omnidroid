...
//! # The Linux mirror
//!
//! This is `tests/arena_execution.rs` on Linux, in a directory target so that
//! `tests/windows_only.rs` (which accounts for every `tests/*.rs` by name) does not see it. The
//! emitted bytes are **unchanged**: they are Microsoft-x64-ABI functions (argument in `ECX`), and
//! Rust calls them as `extern "win64"`, which exists on every x86-64 target -- so the code under
//! test is byte-for-byte what runs on Windows. The executable view is a `MAP_SHARED` view of a
//! memfd; a store through it kills the child with `SIGSEGV`. The one per-process measurement runs in
//! a child process of its own, because on Linux each libtest thread that starts is 2 MiB of
//! `VM_ACCOUNT` -- sixteen times the bound that measurement asserts.
#![cfg(target_os = "linux")]
...
mod x86_64 {
    ...
    unsafe fn call(block: &CodeBlock, argument: u32) -> u32 {
        let function: extern "win64" fn(u32) -> u32 = std::mem::transmute(block.exec_ptr());
        function(argument)
    }
    ...
    /// what would notice.
    /// Runs [`child_two_arenas_emit_chain_seal_patch_and_re_execute_while_the_budget_tracks_the_invisible_half`] alone in a child process, for the reason the module docs give: its
    /// bound on the process counter (a quarter of what was mapped) is smaller than one libtest
    /// thread's stack.
    #[test]
    fn two_arenas_emit_chain_seal_patch_and_re_execute_while_the_budget_tracks_the_invisible_half() {
        ...
        let output = std::process::Command::new(std::env::current_exe().expect("the test binary"))
            .args(["--ignored", "--exact", "x86_64::child_two_arenas_emit_chain_seal_patch_and_re_execute_while_the_budget_tracks_the_invisible_half", "--nocapture"])
            .output()
            .expect("run the child");
        let stdout = String::from_utf8_lossy(&output.stdout);
        eprintln!("{}", String::from_utf8_lossy(&output.stderr));
        assert!(output.status.success(), "the child failed: {}\n{stdout}", output.status);
        assert!(stdout.contains("1 passed"), "the child did not run: {stdout}");
    }

    #[test]
    #[ignore = "run alone in a child process by two_arenas_emit_chain_seal_patch_and_re_execute_while_the_budget_tracks_the_invisible_half"]
    fn child_two_arenas_emit_chain_seal_patch_and_re_execute_while_the_budget_tracks_the_invisible_half() {
        let _serial = SERIAL.lock().unwrap_or_else(|error| error.into_inner());
    ...
    /// Runs [`child_the_measured_cost_of_a_realistic_amount_of_generated_code`] alone in a child
    /// process: see the module docs for why a Linux measurement at this resolution cannot share a
    /// process with other test threads.
    #[test]
    fn the_measured_cost_of_a_realistic_amount_of_generated_code() {
        ...
        let output = std::process::Command::new(std::env::current_exe().expect("the test binary"))
            .args([
                "--ignored",
                "--exact",
                "x86_64::child_the_measured_cost_of_a_realistic_amount_of_generated_code",
                "--nocapture",
            ])
            .output()
            .expect("run the child");
        let stdout = String::from_utf8_lossy(&output.stdout);
        eprintln!("{}", String::from_utf8_lossy(&output.stderr));
        assert!(output.status.success(), "the measurement failed: {}\n{stdout}", output.status);
        assert!(stdout.contains("1 passed"), "the measurement did not run: {stdout}");
    }

    #[test]
    #[ignore = "run alone in a child process by the_measured_cost_of_a_realistic_amount_of_generated_code"]
    fn child_the_measured_cost_of_a_realistic_amount_of_generated_code() {
        ...
        eprintln!(
            "MEASURED: {} MiB of generated code across 2 arenas ({} blocks of {BLOCK} bytes each) \
             cost {} bytes of VM_ACCOUNT and {} bytes of memfd pages that VM_ACCOUNT does not \
             count. Budget: {full}",
            mapped / MIB,
            2 * BLOCKS_PER_ARENA,
            private_delta,
    ...
    fn the_executable_view_is_still_not_writable_after_a_seal_and_unseal_cycle() {
        ...
        assert_eq!(status.signal(), Some(SIGSEGV), "expected SIGSEGV from the child, got {status}");
    }
