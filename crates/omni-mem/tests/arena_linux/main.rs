...
//!
//! **The Linux mirror of `tests/arena.rs`.** A directory target (`tests/arena_linux/main.rs`) so that
//! `tests/windows_only.rs`, which accounts for every `tests/*.rs` by name, does not see it. The
//! section is a `memfd` and the two views are `MAP_SHARED` (see `omni-platform/src/vm/linux.rs`);
//! what changes below is only how a child that stores through the executable view dies (`SIGSEGV`,
//! not `STATUS_ACCESS_VIOLATION`) and how the one per-process measurement is isolated.
#![cfg(target_os = "linux")]
...
fn the_executable_view_is_not_writable() {
    ...
    if std::env::var_os(CHILD).is_some() {
        ...
        // SAFETY: none. This is the point of the test: the store is expected to raise SIGSEGV and
        // kill this process, because the executable view is mapped PROT_READ | PROT_EXEC.
        unsafe {
    ...
    assert_eq!(
        status.signal(),
        Some(libc_sigsegv()),
        "expected the child to die of SIGSEGV, got {status}"
    );
}

/// `SIGSEGV`'s number, which is 11 on every Linux architecture.
fn libc_sigsegv() -> i32 {
    11
}
...
///
/// Reported as a discovery rather than asserted as a budget: the section is *shared* memory (a memfd
/// here), so neither view is `VM_ACCOUNT` and it does not appear in what
...
/// Runs the measurement below in a process of its own. On Linux a test thread's stack is 2 MiB of
/// `VM_ACCOUNT`, so a sibling test *starting* moves `process_commit_charge` by twice this test's
/// tolerance; the child runs exactly one test, so no sibling can start inside the window.
#[test]
fn the_arena_reports_what_it_mapped_even_though_commit_charge_cannot_see_it() {
    let output = std::process::Command::new(std::env::current_exe().expect("the test binary"))
        .args([
            "--ignored",
            "--exact",
            "child_the_arena_commit_measurement",
            "--nocapture",
        ])
        .output()
        .expect("run the child");
...
#[test]
#[ignore = "run in its own process by the_arena_reports_what_it_mapped_even_though_commit_charge_cannot_see_it"]
fn child_the_arena_commit_measurement() {
    ...
    eprintln!(
        "a {} MiB arena chunk: VM_ACCOUNT after mapping {} bytes, after touching every page {} \
    ...
    assert!(
        ...
        "the arena's views should not appear in VM_ACCOUNT as private commit; saw {mapped} bytes"
    );
