...
META_NAME = "meta.json"
# efivars.fd is a UEFI artifact: only the arm base boots through EDK2 pflash.
# An x86-bliss entry boots by direct kernel/initrd and legitimately has no
# such file, so demanding one unconditionally would make every x86 entry fail
# validation and silently cold-boot forever.
CORE_FILES = (STATE_NAME, SYSTEM_NAME, DATA_NAME, META_NAME)
REQUIRED_FILES = CORE_FILES + (EFIVARS_NAME,)      # the arm set


def required_files(arch=None):
    """Files an entry for `arch` must carry to be considered complete.

    An entry whose meta predates the arch field is treated as arm, which is
    what every entry written before x86 support actually was -- so existing
    caches keep validating exactly as before."""
...
def lookup(images_dir, key, qemu_version):
    ...
    try:
        ...
        if not meta_qemu_version or meta_qemu_version != qemu_version:
            return None
        for name in required_files(meta.get("arch")):
            path = entry / name
