...
def data_bake_source(base):
    ...
    An x86-bliss base has NEITHER: it ships a system disk + kernel + initrd
    and no /data of its own, because a fresh x86 instance is seeded from the
    shared empty ext4 template. That template is therefore the pristine image
    an x86 offset overlays, and without this branch the x86 bake would resolve
    its source to None and fail before it booted anything."""
    manifest = base.get("root_manifest") or {}
    recorded = manifest.get("rooted_data") or base.get("data")
    if recorded:
        return recorded
    if base_type(base) == BASE_TYPE_X86:
        return X86_DATA_TEMPLATE
    return None
