...
class QmpSession:
    ...
    def set_migration_caps(self, channels=4, caps=None):
        ...
        `caps` comes from migfile.transport_caps() and is EMPTY on the TCP
        relay: mapped-ram means "write each page at its own offset in the
        destination file", which a stream socket cannot express, and asking
        for it there is not a no-op -- on Windows, `mapped-ram` + `multifd`
        killed the QEMU process outright (measured; see migfile's docstring).
        """
        caps = MIGRATION_CAPS if caps is None else tuple(caps)
        if not caps:
            return
